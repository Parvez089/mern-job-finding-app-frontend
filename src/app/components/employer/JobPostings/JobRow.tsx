  ...
  <tr className="hover:bg-gray-50 transition-colors border-b border-gray-50">
      ...
      <td className="py-5 px-6">
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-lg ${job.avatarBg}`}>
            {job.avatarLetter}
          </div>
          <div>
            <h4 className="font-bold text-[#0e0f1b] mb-0.5">{job.title}</h4>
            <p className="text-xs text-gray-400 font-medium">{job.location} • {job.type}</p>
          </div>
        </div>
      </td>
      ...
      <td className="py-5 px-4 text-center">
        <Tag className="rounded-md border-none bg-gray-50 text-gray-500 font-semibold py-0.5 px-3">
          {job.department}
        </Tag>
      </td>
      ...
      <td className="py-5 px-4 w-[200px]">
        <div className="flex justify-between items-end mb-1">
          <span className="text-xs font-bold text-[#0e0f1b]">{job.candidates} Candidates</span>
          <span className="text-[10px] font-bold text-gray-400">{job.matchPercent}% match</span>
        </div>
        <Progress 
